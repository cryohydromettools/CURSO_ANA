...
infile = '../../../data/AWS_input_AG.nc'   # input path
outfile = './AG_output.nc'               # output path
...
DDFice = 9.0             # melt factor for ice in mm/d/°C
DDFsnow = 6.0            # melt factor for snow in mm/d/°C

Temp_in_K = True        # Is the input temperature given in Kelvin or degrees Celsius?
...
dsout = xr.Dataset(data_vars=dict(
                   ...
                   coords=dict(time=(["time"],time.data), lat=(["lat"],lats.data), lon=(["lon"],lons.data),
                            ),)
